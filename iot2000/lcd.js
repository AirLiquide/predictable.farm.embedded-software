...
var log_file = fs.createWriteStream(__dirname + '/lcd.log', {flags : 'a'});
...
local_service.on('disconnect', (reason) => {
    ...
    datelog(" * LCD is disconnected :"+reason);
    initialScreen();
    //local_service.connect("http://127.0.0.1:" + local_service_PORT, {'forceNew': true, 'force new connection': true});
});

local_service.on('device_id', function (data) {
    datelog(data);
    DEVICE_ID = data;
    local_service.emit('io', "lcd");
});

function _send_scheduled() {
    local_service.emit("scheduled", "lcd");  
}


var cmd_process = [];
var busy = false;
function lcd_print_now() {
   // datelog("lcd_print_now " + cmd_process.length);
    if(busy)
    {
        datelog("lcd_print_now busy" );
        return;
    } 
...
local_service.on('lcd', function (data) {
    //datelog(data);
    try {
...
local_service.on('scheduler', function (data) {
    ...
    {
        //datelog("scheduled");
        while (cmd_process.length > 0) lcd_print_now();
